...
grammar = {
   ...
   /function()
      if gblCurEle then
         print("end:",gblCurEle.id)
      end
   end,
   Type = P"media"+"context"+"area"+"region"+"macro",
   ReservedCondition = P"onBegin"+P"onEnd",
   ReservedAction = P"start"+P"stop",
   Reserved = V"Type"+V"ReservedAction"+V"ReservedCondition"+P"do",
   Id = R("az", "AZ", "__") * V"Alnum"^0,
   PropertyValue = (V"Letter"+V"Num")^1 + P"\""*(V"Letter"+V"Num")^0*P"\"",
   ...
   *(V"PresentationElement" + V"Property"+V"Spc")^0 *C(V"End")),
   ...
   ConditionId = sT.makeRelationship(C(V"Reserved") *V"Spc"^1* (C(V"Id")*(P"."*C(V"Id"))^-1), "condition"),
   ...
   ActionId = sT.makeRelationship(C(V"Reserved") *V"Spc"^1* (C(V"Id")*(P"."*C(V"Id"))^-1),"action"),
